...
rule cutadapt:
  ...
  shell:
    "cutadapt {ADAPTOR_SEQUENCE} -o  {output.first_file} -p  {output.second_file} {input.first_file} {input.second_file} > {log}"
...
rule urqt:
  ...
  shell:
    "{URQT} --m {URQT_THREADS} --t 20 --gz --in {input.first_file} --inpair {input.second_file} --out {output.urqt_trimmed_1} --outpair {output.urqt_trimmed_2} > {log}"

rule fastqc_untrim:
  input:
    untrimmed_fastq_1 = "{path}/data/{files}_1.fastq.gz",
    untrimmed_fastq_2 = "{path}/data/{files}_2.fastq.gz",
  ...
  shell:
    "fastqc --quiet  {FASTQ_THREADS} --outdir {output} {input.untrimmed_fastq_1} {input.untrimmed_fastq_2}"
...
rule fastqc_trim:
  ...
  shell:
    "fastqc --quiet {FASTQ_THREADS} --outdir {output} {input.trimmed_urqt_1} {input.trimmed_urqt_2}"
